...
$apiSln = "$PSScriptRoot\src\SeedWork.sln"

Write-Host "Cleaning TestResult folders..."
Get-ChildItem -Path "$PSScriptRoot\src\" -Recurse -Directory -Include TestResults | Remove-Item -Recurse

Write-Host "Restoring dotnet tools..."
dotnet tool restore

Write-Host "Running all tests [Debug Mode] ..."
$testResultsFolder = "$PSScriptRoot\src\Todo.Tests\TestResults";
...
$testCoverageFolder = "$PSScriptRoot\src\Todo.Tests\Coverage";
